...
```
### Просмотр Swagger UI для OpenAPI (Синхронное API), cгенерируемого SpringBoot 
По умолчанию откроется локальная схема и будет "Explorer" куда можно скопировать ссылки на дургие OpenAPI схемы
```bash
open http://localhost:8080/swagger-ui.html
```

### Просмотр внешнего Swagger UI для OpenAPI (Синхронное API) для нескольких приложений 
...
### Просмотр AsyncAPI (Асинхронное API), cгенерируемого SpringBoot через Springwolf
JSON Спецификация 
```bash
open http://localhost:8080/springwolf/docs
```
AsyncAPI UI
```bash
